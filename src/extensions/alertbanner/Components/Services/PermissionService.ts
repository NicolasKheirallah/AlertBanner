...
export class PermissionService {
  ...
  public async validateAllPermissions(): Promise<IPermissionStatus[]> {
    const now = Date.now();
    if (
      now - this.lastPermissionCheck < this.PERMISSION_CACHE_TTL &&
  ...
  private async checkPermission(
    ...
    try {
      ...
      const is403 = err.statusCode === 403 || err.code === "Forbidden";
      const is401 = err.statusCode === 401 || err.code === "Unauthorized";
  ...
  public logAuditableOperation(operation: {
    ...
    logger.info(
      "AUDIT",
      `Auditable operation: ${operation.operation}`,
      auditEntry,
    );
  ...
  public async executeWriteOperation<T>(
    ...
    const enrichedAudit = {
      ...audit,
      targetSiteUrl:
        audit.targetSiteUrl || this.context.pageContext.web.absoluteUrl,
      targetSiteName:
        audit.targetSiteName || this.context.pageContext.web.title,
    };

    const canWrite = await this.canWriteSites();
    if (!canWrite) {
      const siteContext =
        enrichedAudit.targetSiteName ||
        enrichedAudit.targetSiteUrl ||
        "target site";
      ...
      logger.error(
        "PermissionService",
        "Write operation blocked - permission not granted",
        {
  ...
  public getAdminConsentUrl(): string {
    ...
    const clientId =
      this.context.pageContext.aadInfo?.instanceId ||
  ...
  private getCorrelationId(): string {
    return (
      (this.context as unknown as Record<string, string>).correlationId ||
      `${Date.now()}-${Math.random().toString(36).substring(2, 11)}`
    );
  }

  private sendToAuditLog(entry: Record<string, unknown>): void {
