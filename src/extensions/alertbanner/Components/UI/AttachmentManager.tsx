...
const UploadProgressFill: React.FC<IUploadProgress> = ({
  progress,
  status,
}) => {
...
const AttachmentManager: React.FC<IAttachmentManagerProps> = ({
  ...
  const uploadAttachment = React.useCallback(
    async (
      ...
      try {
        logger.info("AttachmentManager", "Starting attachment upload", {
          fileName: file.name,
          itemId,
        });
        ...
        const result = await alertService.addAttachment(
          listId,
          itemId,
          file.name,
          arrayBuffer,
        ...
        logger.info("AttachmentManager", "Attachment uploaded successfully", {
    ...
    [listId, itemId, siteId, alertService],
  );
    ...
    async (filesToProcess: File[]) => {
      if (!itemId) {
        notificationService.showWarning(
          strings.AttachmentManagerSaveAlertFirst,
          strings.AttachmentManagerTitle,
        );
        return;
      }
      ...
      filesToProcess.forEach((file) => {
        ...
        if (!allowedFormats.includes(fileExtension)) {
          errors.push(
            Text.format(
              strings.AttachmentManagerInvalidFormat,
              file.name,
              allowedFormats.join(", "),
            ),
          );
          ...
          errors.push(
            Text.format(
              strings.AttachmentManagerFileTooLarge,
              file.name,
      ...
      try {
        for (let i = 0; i < validFiles.length; i++) {
          ...
          try {
            ...
            logger.error("AttachmentManager", "Failed to upload file", {
              fileName: file.name,
              error,
            });
          }
        ...
        logger.error(
          "AttachmentManager",
          "Attachment upload process failed",
          error,
        );
        notificationService.showError(
          strings.AttachmentManagerUploadFailed,
          strings.AttachmentManagerTitle,
        );
      } finally {
  ...
  const handleDrop = React.useCallback(
    ...
    [disabled, isUploading, itemId, processFiles],
  );
  ...
  const handleRemoveAttachment = React.useCallback(
    async (attachment: IAttachment) => {
      ...
      const shouldDelete = await confirm({
        ...
        message: Text.format(
          strings.AttachmentManagerDeleteConfirm,
          attachment.fileName,
        ),
      ...
      try {
        await alertService.deleteAttachment(
          listId,
          itemId,
          attachment.fileName,
          siteId,
        ...
        logger.info("AttachmentManager", "Attachment deleted successfully", {
          fileName: attachment.fileName,
        });
      } catch (error) {
        logger.error("AttachmentManager", "Failed to delete attachment", error);
        notificationService.showError(
          strings.AttachmentManagerDeleteError,
          strings.AttachmentManagerTitle,
        );
      }
    },
  ...
  return (
    <div className={styles.attachmentManager}>
      ...
      <div className={styles.header}>
        ...
        >
          {isUploading
            ? strings.AttachmentManagerUploadingLabel
            : strings.AttachmentManagerAddFilesButton}
        </PrimaryButton>
      </div>
      ...
      {itemId && !isUploading && (
        ...
        >
          ...
          <div className={styles.dropZoneHint}>
            {Text.format(
              strings.AttachmentManagerDropZoneHint,
              allowedFormats.join(", "),
              maxFileSize.toString(),
            )}
          </div>
      ...
      {uploadProgress.length > 0 && (
        <div className={styles.progressContainer}>
          <div className={styles.progressTitle}>
            {strings.AttachmentManagerProgressTitle}
          </div>
              ...
              <div className={styles.progressHeader}>
                ...
                <span className={styles.progressStatus}>
                  {progress.status === "completed" &&
                    strings.AttachmentManagerStatusCompleted}
                  {progress.status === "error" &&
                    strings.AttachmentManagerStatusFailed}
      ...
      <div className={styles.helpText}>
        {Text.format(
          strings.AttachmentManagerHelpText,
          allowedFormats.join(", "),
          maxFileSize.toString(),
        )}
      </div>
