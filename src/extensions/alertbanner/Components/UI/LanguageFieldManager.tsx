...
const LanguageFieldManager: React.FC<ILanguageFieldManagerProps> = ({
  ...
  const [languages, setLanguages] =
    React.useState<ILanguage[]>(DEFAULT_LANGUAGES);
  ...
  const { loading, execute: loadSupportedLanguages } = useAsyncOperation(
    async () => {
      ...
      logger.debug(
        ...
        `Site default language detected: ${siteDefaultLanguage}`,
      );
      logger.debug(
        "LanguageFieldManager",
        `Supported languages from SharePoint: ${supported.join(", ")}`,
      );
      ...
      const updatedLanguages = supportedLanguages.map((stdLang) => {
        ...
        return {
          ...
          flag: stdLang.flag,
          isAdded:
            supported.includes(stdLang.code) ||
            (stdLang.code === siteDefaultLanguage && supported.length === 0),
      ...
      if (supported.length === 0) {
        ...
        logger.debug(
          "LanguageFieldManager",
          `Set ${siteDefaultLanguage} as default language for new installation`,
        );
    ...
    },
    {
      ...
      onError: () => {
        ...
        showMessage(
          "warning",
          CoreText.format(
            strings.LanguageManagerLoadFailedDefault,
            siteDefaultLanguage,
          ),
        );
      },
  ...
  const handleLanguageToggle = async (
    ...
    if (languageCode !== "en-us") {
      ...
      if (languageCode === siteDefaultLanguage && !checked) {
        const defaultLanguageName =
          languages.find((l) => l.code === siteDefaultLanguage)?.name ||
          "default";
        showMessage(
          "warning",
          CoreText.format(
            strings.LanguageManagerDefaultLanguageProtected,
            defaultLanguageName,
          ),
        );
        return;
    ...
    try {
      if (checked) {
        ...
        showMessage(
          "success",
          CoreText.format(strings.LanguageManagerAddedSuccess, language.name),
        );
      } else {
        setIsTogglingLanguage(true);
        ...
        showMessage(
          "success",
          CoreText.format(strings.LanguageManagerRemovedSuccess, language.name),
        );
      }
      ...
      const activeLanguages = languages
        .filter(
          (l) =>
            (l.code === languageCode ? checked : l.isAdded) && !l.isPending,
        )
        .map((l) => l.code);
      ...
      logger.error(
        "LanguageFieldManager",
        `Failed to ${checked ? "add" : "remove"} language ${languageCode}`,
        error,
      );
      showMessage(
        "error",
        CoreText.format(strings.LanguageManagerUpdateFailed, language.name),
      );
  ...
  const activeLabel =
    pendingCount > 0
      ? CoreText.format(
          strings.LanguageManagerActiveCountWithPending,
          addedCount,
          pendingCount,
        )
    ...
    <div className={styles.container}>
      ...
      <Card>
        <CardHeader
          ...
          description={
            <Text size={200}>{strings.LanguageManagerDescription}</Text>
          }
        />
            ...
            <div className={styles.languageHeader}>
              <div className={styles.languageSummary}>
                <Text weight="semibold">
                  {strings.LanguageManagerAvailableLanguages}
                </Text>
