...
| ------- | ------------------ | ----------------------------------------------- |
| 3.2 | October 16, 2025 | Fixed critical carousel settings page reload/crash bug with event-based state management, implemented proper color contrast for alert previews (WCAG AAA compliance), fixed SiteContextDetector with proper Graph API site ID extraction and hub site detection, comprehensive code cleanup, , enhanced error handling in multi-site operations, production-ready TypeScript improvements with explicit typing |
